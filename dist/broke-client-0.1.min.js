...
(function(){var gettext=broke.i18n.gettext;broke.Class.extend("broke.db.models.Query",{exclude:function(args){return this.filter(args,false)},create:function(args){this.storage.push(args);return(new this.model(args)).save()},getOrCreate:function(args){try{return this.get(args)}catch(e){if(e.name==="DoesNotExist")return this.create(args);throw e;}},get:function(args){var object=null;if(args)object=this.filter(args).asObject();else object=this.asObject();if(object.length>1)throw this.model.MultipleObjectsReturned(gettext("get() returned few %s instances -- it returned %s! Lookup parameters were %s").echo(this.model.className,
...
(function(){var tpl=broke.template,gettext=broke.i18n.gettext,TemplateSyntaxError=broke.exceptions.TemplateSyntaxError;broke.Class.extend("broke.template.Parser",{init:function(tokens){this.tokens=tokens},parse:function(parseUntil){var nodelist=[],token=null,tagFuncName=null,tagFunc=null;if(!parseUntil)parseUntil=[];if(!(parseUntil instanceof Array))parseUntil=[parseUntil];while(this.tokens.length){token=this.tokens.shift();if(token.type===tpl.TOKEN_TEXT)nodelist.push(new tpl.TextNode(token.content));else if(token.type===
...
(function(){var tpl=broke.template,register=tpl.register,TemplateSyntaxError=broke.exceptions.TemplateSyntaxError,gettext=broke.i18n.gettext;register.tag("if",function(parser,token){var bits=token.content.split(/\s+/),linkType,nodeListTrue,nodeListFalse;bits.shift();if(!bits)throw TemplateSyntaxError(gettext("If node need at least one args"));var bitstr=bits.join(" "),boolPairs=bitstr.split(" or ");if(boolPairs.length==1){linkType=tpl.IfNode._and;boolPairs=bitstr.split(" and ")}else{linkType=tpl.IfNode._or;
...
(function(){var ngettext=broke.i18n.ngettext,gettext=broke.i18n.gettext,defaultFilters={add:function(value){return value.asInt()+value.asInt()},addslashes:function(value){return value.replace("\\","\\\\").replace('"','\\"').replace("'","\\'")},capfirst:function(value){return value[0].upper()+value.slice(1).lower()},center:function(value,args){return center(value,args.asInt())},cut:function(value,args){return value.replace(args,"")},date:function(value,args){if(typeOf(value)!=="date")value=new Date(value);
